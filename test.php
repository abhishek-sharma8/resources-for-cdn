...
function storefront_child_enqueue_styles() {
    ...
    wp_enqueue_style('slick', 'https://cdnjs.cloudflare.com/ajax/libs/slick-carousel/1.8.1/slick.min.css', false);
    wp_enqueue_style('slick-theme', 'https://cdnjs.cloudflare.com/ajax/libs/slick-carousel/1.8.1/slick-theme.min.css', false);
...
function seccow_send_email($order_id, $old_status, $new_status, $order) {
    if ($new_status == 'failed') {
        ...
        $wc_emails['WC_Email_Failed_Order']->recipient = bloginfo('admin_email');//'devgad@devgadmango.com';
...
$image_url = $image[0];
            ...
            <tbody>
                                                                ...
                                                                <tr>
                                                                    <td style="padding: 48px 48px 32px;" valign="top">
                                                                        <div id="body_content_inner"
                                                                            ...
                                                                            <p style="margin: 0 0 16px;">'.get_bloginfo( 'name' ).'</p>
                ...
                <tr>
                    <td align="center" valign="top"><!-- Footer -->
                        ...
                        <tbody>
                        <tr>
                        <td style="margin: 0 0; text-align: left;"><a class="link" style="font-weight: normal; text-decoration: underline; color: #ffb300;" href="'.site_url("about-us").'">About Us</a> / <a class="link" style="font-weight: normal; text-decoration: underline; color: #ffb300;" href="'.site_url("/product/mango/").'">Order Mangoes</a></td>
                        <td style="margin: 0 0; text-align: right;">©'.date('Y').' '.get_bloginfo( 'name' ).'</td>
                        </tr>
                        </tbody>
                        </table>
                        <!-- End Footer --></td>
        ...
        wp_mail($email_cliente, get_bloginfo( 'name' ).' Order failed! You are just one step away!', $emsg, $headers);
...
function wc_cart_item_name_hyperlink($link_text, $product_data) {
    ...
    if ($title == 'Order Konkan Mangoes') {
        $link_text = 'Konkan Mangoes';
    }
    return $link_text;
}
...
function pmnm_button_funct($attr) {
    ...
    if (isset($attr["text"]) && !empty($attr["text"])) {
        ...
        $text = 'Click Here to Order Konkan Mango';
    }
...
function nm_action_woocommerce_before_cart_table(  ) { 
    ...
    echo '<p style="margin-top:10px;">Enjoy the original Konkan Aamras! <b>Delivery starting from 7th July 2022.</b></p>';
}; 
